...
- Правило истощения:
- каждые `3` секунды без еды змея теряет `1` сегмент;
- если сегментов меньше `4`, змея движется очень медленно;
- после `3` уменьшений длины подряд — `DNF (истощение)`.
...
- Игровая область (`race-stage`) увеличена (крупнее на десктопе и мобильных), чтобы не требовался постоянный zoom браузера.
